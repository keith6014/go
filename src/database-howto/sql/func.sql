...
$BODY$
BEGIN
  -- INSERT INTO author (name) values ((select unnest(xpath('/attendee/bio/name/text()',data)) from xmltest order by id limit 1));
  TRUNCATE author;
  INSERT INTO author (name) ((select unnest(xpath('/attendee/bio/name/text()',data)) from xmltest ));
